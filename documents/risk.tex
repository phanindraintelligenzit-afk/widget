...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=gray!5!white, colframe=dpiblue, boxrule=1pt, arc=3mm, 
    title={\textbf{\faCalculator\ Formula Variables}}, fonttitle=\bfseries]
\begin{itemize}
    \item \textbf{\color{dpiblue}\faListUl\ Incidents ($\sum$):} The summation of all unique incident types reported during the evaluation period.
    \item \textbf{\color{dpigreen}\faSyncAlt\ Frequency:} The number of times a specific incident occurred.
    \item \textbf{\color{dpired}\faExclamationTriangle\ Severity:} The base severity weight (e.g., Low=1, Medium=3, High=5, Critical=10).
    \item \textbf{\color{dpicloud}\faTachometerAlt\ $R_{max}$:} The maximum risk threshold defined in the engine settings.
\end{itemize}
\end{tcolorbox}
\vspace{0.3cm}
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=white, colframe=dpiteal, boxrule=1pt, arc=2mm, title={\textbf{\faClipboardList\ Incident Log (24 Hours)}}, fonttitle=\bfseries]
\begin{itemize}
    \item \textbf{\color{dpicloud}\faBug\ Incident A (API Throttling):} Occurred \textbf{5} times. Severity: \textbf{Low (2)}. \hfill $\rightarrow \mathbf{(5 \times 2) = 10}$
    \item \textbf{\color{dpired}\faUserShield\ Incident B (Unauthorized DB Access Attempt):} Occurred \textbf{1} time. Severity: \textbf{Critical (10)}. \hfill $\rightarrow \mathbf{(1 \times 10) = 10}$
    \item \textbf{\color{dpicloud}\faCode\ Incident C (JSON Parsing Error):} Occurred \textbf{3} times. Severity: \textbf{Low (1)}. \hfill $\rightarrow \mathbf{(3 \times 1) = 3}$
\end{itemize}
\end{tcolorbox}
\vspace{0.3cm}
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=white, colframe=dpired, boxrule=1pt, arc=2mm, title={\textbf{\faClipboardList\ Incident Log (Runaway Loop)}}, fonttitle=\bfseries]
\begin{itemize}
    \item \textbf{\color{dpired}\faFire\ Incident D (Runaway API Loop):} Occurred \textbf{15} times. Severity: \textbf{High (5)}. \hfill $\rightarrow \mathbf{(15 \times 5) = 75}$
\end{itemize}
\end{tcolorbox}
\vspace{0.3cm}
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpiblue!5!white, colframe=dpiblue, title={\textbf{\faClipboardList\ Jira Service Management (JSM)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpiblue}What it is:} Atlassian's premier Incident Ticket System Management solution used for tracking bugs, tasks, and operational incidents. \\[0.2cm]
\textbf{\color{dpiblue}Integration:} Jira Automation rules are configured to monitor the \texttt{Agent-Incidents} project. Whenever a high-priority bug or compliance failure is logged against an AI agent, Jira automatically triggers a webhook to DPI-LS to incrementally lower the Risk score.
\end{tcolorbox}


\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpigreen!5!white, colframe=dpigreen, title={\textbf{\faServer\ ServiceNow (SNOW)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpigreen}What it is:} The leading enterprise cloud platform for IT workflows and comprehensive service management. \\[0.2cm]
\textbf{\color{dpigreen}Integration:} ServiceNow manages the complete lifecycle of a critical outage. If an AI agent hallucinates or crashes a downstream system, a P1 or P2 Incident is raised in SNOW. The DPI-LS integration maps SNOW's Urgency and Impact matrices directly to DPI-LS severity weights.
\end{tcolorbox}


\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpicloud!5!white, colframe=dpicloud, title={\textbf{\faHeadset\ Zendesk \& Freshservice}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpicloud}What it is:} Customer-facing support platforms where end-users report bot failures, unhelpful responses, or reputational risks. \\[0.2cm]
\textbf{\color{dpicloud}Integration:} Customer support teams tag specific user complaints with "Bot-Failure" or "Hallucination." When these tags are applied, the platforms use their native Trigger/Webhook features to inform DPI-LS of the reputational damage, lowering the agent's R-score.
\end{tcolorbox}
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpired!5!white, colframe=dpired, title={\textbf{\faChartLine\ Datadog / Dynatrace / New Relic}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpired}What it is:} Essential monitoring platforms for cloud applications, specializing in Application Performance Monitoring (APM). \\[0.2cm]
\textbf{\color{dpired}Integration:} These APMs act as the primary health monitor. If the AI agent enters a failure loop (e.g., repeating the same LLM call hundreds of times per minute) or if its response latency spikes beyond acceptable SLAs, the APM triggers an anomaly monitor via Webhook to DPI-LS.
\end{tcolorbox}


\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpired!5!white, colframe=dpired, title={\textbf{\faBug\ Sentry / Splunk / Elastic}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpired}What it is:} Application monitoring and SIEM platforms built to capture unhandled exceptions, crashes, software bugs, and log anomalies. \\[0.2cm]
\textbf{\color{dpired}Integration:} These tools capture silent failures that might not trigger a full outage but still degrade the agent's reliability (e.g., the LLM returns an unparseable JSON block that causes a local parser crash). Splunk and Elastic SIEMs can also detect suspicious agent log behaviors.
\end{tcolorbox}

\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpicloud!5!white, colframe=dpicloud, title={\textbf{\faCloud\ Cloud-Native Integrations (AWS CloudWatch, Azure Monitor)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpicloud}What it is:} Native telemetry and billing monitors provided directly by the hyperscalers. \\[0.2cm]
\textbf{\color{dpicloud}Integration:} Autonomous agents can generate massive bills if they hit runaway loops using expensive foundational models. Configuring AWS Budgets attached to an SNS topic allows DPI-LS to immediately flag a "Runaway Cost" incident and trip the safety breaker before the bill escalates further.
\end{tcolorbox}

\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpiteal!5!white, colframe=dpiteal, title={\textbf{\faUserShield\ Security Posture \& AppSec (Prisma Cloud, Wiz, Checkmarx)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpiteal}What it is:} Cloud Security Posture Management (CSPM) and Application Security platforms. \\[0.2cm]
\textbf{\color{dpiteal}Integration:} If an autonomous agent attempts to execute unauthorized commands or touches protected infrastructure, platforms like Prisma Cloud or Wiz immediately detect the policy violation. Checkmarx can scan the agent's dynamic code generation for zero-day vulnerabilities. These platforms fire Critical Severity webhooks directly to DPI-LS, terminating the agent instantly.
\end{tcolorbox}

\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=dpiblue!5!white, colframe=dpiblue, title={\textbf{\faPhoneVolume\ Incident Response \& Paging (PagerDuty, Opsgenie)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\textbf{\color{dpiblue}What it is:} Automated incident routing and on-call paging platforms. \\[0.2cm]
\textbf{\color{dpiblue}Integration:} If the DPI-LS R-Score drops to 0.0, DPI-LS can trigger a High-Urgency PagerDuty or Opsgenie incident, immediately waking up the on-call engineer to review the blocked agent's actions.
\end{tcolorbox}

\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=gray!5!white, colframe=gray, title={\textbf{\faLayerGroup\ Specialized Risk Platforms (FinOps, Data Quality, IAM, Code Quality)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\begin{itemize}
\item \textbf{\color{dpicloud}\faMoneyBillWave\ FinOps (Cloudability, Finout):} Tracks deep, unit-level agent spend across multiple LLM providers (OpenAI, Anthropic, Gemini). Triggers DPI-LS webhooks on unexpected cost spikes.
    \item \textbf{\color{dpigreen}\faDatabase\ Data Quality (Monte Carlo, Bigeye):} Detects if an agent begins pumping garbage or hallucinatory data into a Snowflake/BigQuery data warehouse.
    \item \textbf{\color{dpiblue}\faIdCard\ Identity \& Access Management (Okta, Ping Identity):} Detects if an agent's Service Account starts brute-forcing logins or attempting impossible-travel authentication.
    \item \textbf{\color{dpired}\faCode\ Code Quality / SAST (SonarQube):} Scans code written by software engineering agents and fires webhooks if Critical or Blocker vulnerabilities are introduced into the PR.
\end{itemize}
\end{tcolorbox}
...
\begin{tikzpicture}[
    ...
    step2/.style={rectangle, draw=dpiblue, fill=dpiblue!5, thick, text width=4.5cm, align=center, minimum height=3cm, rounded corners, drop shadow},
    step3/.style={rectangle, draw=dpigreen, fill=dpigreen!5, thick, text width=4.5cm, align=center, minimum height=3cm, rounded corners, drop shadow},
    arrow/.style={-{Stealth[scale=1.5]}, thick, dpicloud}
]

\node[step1] (s1) {\textcolor{gray}{\Large\faSearch}\\[0.2cm]\textcolor{gray}{\textbf{Phase 1: APM Baselines}}\\[0.2cm]\small \faCode\ Deploy Sentry SDKs \\ \faClock\ Configure Datadog \\ \faUserSecret\ Run in "Shadow Mode"};
\node[step2, right=of s1] (s2) {\textcolor{dpiblue}{\Large\faServer}\\[0.2cm]\textcolor{dpiblue}{\textbf{Phase 2: ITSM Alignment}}\\[0.2cm]\small \faUsers\ SNOW "AI-Agent" CI \\ \faCogs\ Jira Automation rules \\ \faSlidersH\ Align $R_{max}$ threshold};
\node[step3, right=of s2] (s3) {\textcolor{dpigreen}{\Large\faCheckCircle}\\[0.2cm]\textcolor{dpigreen}{\textbf{Phase 3: Active Mitigation}}\\[0.2cm]\small \faUserShield\ Turn off "Shadow Mode" \\ \faBolt\ Active API revocation \\ \faMoneyBillWave\ AWS Billing Alerts};

\draw[arrow] (s1) -- (s2);
\draw[arrow] (s2) -- (s3);

% Timeline bar underneath
\draw[line width=3pt, gray] ([yshift=-0.2cm, xshift=-1cm]s1.south west) -- ([yshift=-0.2cm]s1.south);
\draw[line width=3pt, dpiblue] ([yshift=-0.2cm]s1.south) -- ([yshift=-0.2cm]s2.south);
\draw[line width=3pt, dpigreen] ([yshift=-0.2cm]s2.south) -- ([yshift=-0.2cm, xshift=1cm]s3.south east);

\node[circle, fill=gray, text=white, inner sep=1.5pt, font=\tiny\bfseries] at ([yshift=-0.2cm]s1.south) {1};
\node[circle, fill=dpiblue, text=white, inner sep=1.5pt, font=\tiny\bfseries] at ([yshift=-0.2cm]s2.south) {2};
\node[circle, fill=dpigreen, text=white, inner sep=1.5pt, font=\tiny\bfseries] at ([yshift=-0.2cm]s3.south) {3};

\node[below=0.3cm of s1, text=gray, font=\bfseries] {Weeks 1-4};
\node[below=0.3cm of s2, text=dpiblue, font=\bfseries] {Weeks 5-8};
\node[below=0.3cm of s3, text=dpigreen, font=\bfseries] {Weeks 9-12};
...
\draw[arrow] (snow) -- node[below, font=\footnotesize] {REST Webhook} (dpils);
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=gray!5!white, colframe=gray, title={\textbf{\faRobot\ 2. Agent Thought Process \& Timeline}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=blue!5!white, colframe=dpiblue, title={\textbf{\faCogs\ 4. Integration \& Score Downgrade}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
\begin{itemize}
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=green!5!white, colframe=dpigreen, title={\textbf{\faWrench\ 6. Root Cause Analysis \& Resolution}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
...
\draw[arrow] (aws) -- node[below, font=\footnotesize] {SNS Topic POST} (dpils);
...
\draw[arrow] (user) -- node[sloped, below, pos=0.35, font=\footnotesize, fill=white, inner sep=2pt] {Emails Support} (zendesk);
\draw[arrow] (zendesk) -- node[below, font=\footnotesize] {Trigger Webhook} (dpils);
\draw[arrow, dashed, draw=dpired] (dpils) -- node[sloped, above, pos=0.65, font=\footnotesize, fill=white, inner sep=2pt] {Log Trend} (bot);
\end{tikzpicture}
}
\end{center}
\vspace{0.3cm}
...
\draw[arrow] (agent) -- node[sloped, above, font=\footnotesize] {Send Traces} (ddog);
\draw[arrow] (ddog) -- node[below, font=\footnotesize] {Anomaly Webhook} (dpils);
\end{tikzpicture}
}
\end{center}
\vspace{0.3cm}
...
\draw[arrow] (jira) -- node[below, font=\footnotesize] {Automation Webhook} (dpils);
...
\draw[arrow] (prisma) -- node[below, font=\footnotesize] {Critical Webhook} (dpils);
...
\vspace{0.3cm}
\begin{tcolorbox}[enhanced, colback=gray!5!white, colframe=gray, title={\textbf{\faBriefcase\ Business Capabilities for the R-Dimension}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm]
Integrating both ITSM and Observability platforms into DPI-LS provides:
\begin{itemize}
    \item \textbf{\color{dpiblue}\faUserShield\ Defense in Depth}: Aligns the agent's Risk score with formal human ticketing systems (Jira, SNOW) while retaining the millisecond reaction times of automated APM tools (Datadog, Sentry).
    \item \textbf{\color{dpigreen}\faChartBar\ Quantitative Risk Management}: Translates abstract support tickets, bugs, and API telemetry into a single standardized, mathematical risk score ($R$).
    \item \textbf{\color{dpired}\faBolt\ Automated Safety Nets}: Allows dynamic suspension of runaway agents when critical P1 tickets breach the $R_{max}$ threshold.
\end{itemize}
\end{tcolorbox}
\vspace{0.3cm}
...
\node[step1] (s1) {\textcolor{dpiblue}{\Large\faFileCode}\\[0.2cm]\textcolor{dpiblue}{\textbf{Step 1}}\\[0.2cm]\small Finalize Ticket Schemas for Jira, ServiceNow, and Datadog payloads};
\node[step2, right=of s1] (s2) {\textcolor{dpiteal}{\Large\faSlidersH}\\[0.2cm]\textcolor{dpiteal}{\textbf{Step 2}}\\[0.2cm]\small Calibrate $R_{max}$ to establish the organizational risk appetite baseline};
\node[step3, right=of s2] (s3) {\textcolor{dpigreen}{\Large\faChartLine}\\[0.2cm]\textcolor{dpigreen}{\textbf{Step 3}}\\[0.2cm]\small UI Integration: Display clickable trace links directly on the dashboard};
...
\textcolor{dpicloud}{\small Digital Performance Index --- Risk Dimension (R) --- Version 1.0}\\[0.1cm]
...
\textcolor{white}{\textbf{\Large AI Team}}\\[0.15cm]
\textcolor{white}{\textbf{\Large Malkapurapu Sivamanikanta}}\\[0.3cm]
\textcolor{dpicloud}{\small Digital Performance Index --- Risk Dimension (R) --- Version 2.5}\\[0.1cm]
\textcolor{dpicloud}{\small Architectural Reference $\bullet$ Confidential}
\end{center}
\end{tcolorbox}
