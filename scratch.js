...
let dashboardUrls = {
  ...
  'Jaeger':        { url: 'http://' + window.location.hostname + ':16686', online: false },
  'Zipkin':        { url: 'http://' + window.location.hostname + ':9411', online: false },
  ...
  'OpenTelemetry': { url: 'http://' + window.location.hostname + ':16686', online: false },

  'Apache SkyWalking': { url: 'http://' + window.location.hostname + ':8080', online: false },
  'Keycloak':      { url: 'https://www.keycloak.org/documentation', online: true },
  'OpenMetadata':  { url: 'https://docs.open-metadata.org', online: true },
};
    ...
    const processUrls = async (r) => {
      if (r.ok) {
        ...
        for (const [name, val] of Object.entries(raw)) {
          let u = (val && typeof val === 'object' && 'url' in val) ? val.url : String(val);
          let online = (val && typeof val === 'object' && 'online' in val) ? val.online : false;
          if ((!u || u === '#') && RESOURCE_META[name] && RESOURCE_META[name].dashboard_url) {
              u = RESOURCE_META[name].dashboard_url;
          }
          dashboardUrls[name] = { url: u, online: online };
        }
      }
    };
    await Promise.all([
...
const RESOURCE_META = {
  ...
  "OpenMetadata": {
    name: 'OpenMetadata',
    icon: '📊', category: 'Governance',
    description: 'Open standard for metadata, search, and lineage.',
    tags: ['Metadata', 'Lineage', 'Catalog']
  },
  'Falco': {
    icon: '🦅', category: 'Risk & Security',
    dpi_status: 'primary',
    description: 'Cloud native runtime security focusing on system calls and anomalies.',
    dashboard_url: 'https://falco.org/',
    dashboard_label: 'Open Falco',
    color: '#06b6d4',
    sdk: 'falco'
  },
  'Sentry': {
  ...
  'OpenLIT': {
    ...
    dashboard_url: 'http://' + window.location.hostname + ':3000',
  ...
  'OpenCost': {
    ...
    dashboard_url: 'http://' + window.location.hostname + ':9003',
  ...
  'Jaeger': {
    ...
    dashboard_url: 'http://' + window.location.hostname + ':16686',
...
function resourceStatus(name, metrics = []) {
  ...
  if (metrics.length === 0) return 'failed';
  
  if (metrics.some(m => m.status === 'CREDENTIALS_MISSING' || m.status === 'FAILED')) return 'failed';
...
function renderResourceTable(metrics, name) {
  ...
  if (['OpenCost', 'OpenLIT'].includes(name)) {
  ...
  } else if (['Langfuse', 'OpenTelemetry', 'Apache SkyWalking'].includes(name)) {
  ...
  } else if (['DeepEval', 'Jaeger', 'Zipkin', 'Guardrails AI', 'Pydantic AI', 'Instructor'].includes(name)) {
  ...
  } else if (['Open Policy Agent', 'Microsoft Presidio', 'Detect-Secrets', 'Keycloak', 'OpenMetadata'].includes(name)) {
...
function renderResourceCard(name, metrics = []) {
  ...
  const DOCS = {
    ...
    "Sentry": "https://develop.sentry.dev/",
  ...
  const docHtml = `<a class="rc-dashboard-link-inline" href="${docUrl}" target="_blank" style="color:var(--muted); font-size:12px; text-decoration:underline;">Documentation</a>`;

  const libraryBased = ['DeepEval', 'Ragas', 'Guardrails AI', 'Pydantic AI', 'Instructor', 'Detect Secrets', 'Microsoft Presidio', 'Workflow Layer', 'TruLens', 'LLM Guard', 'Rebuff'];
  let linkHtml;
  if (libraryBased.includes(name)) {
    linkHtml = `<span style="color:var(--muted2); font-weight:700; font-size:13px;">LIBRARY-BASED / NO STANDALONE URL</span>`;
  } else {
    const isOffline = !dashUrl || dashUrl === '#' || (!tcpOnline && !['LangSmith', 'AgentOps', 'TraceLoop', 'Langfuse', 'Confident AI'].includes(name));
    if (isOffline) {
      linkHtml = `<span style="color:var(--muted2); font-weight:700; font-size:13px;">⚠️ Service Offline / Not Configured</span>`;
    } else {
      linkHtml = `<a class="rc-dashboard-link-inline" href="${dashUrl}" target="_blank" style="color:var(--accent); font-weight:700; text-decoration:underline; font-size:13px; display:inline-flex; align-items:center; gap:4px;">🔗 Open Resource</a>${liveTag}`;
    }
  }

  linkHtml = linkHtml + `<span style="margin: 0 8px; color: var(--muted);">|</span>` + docHtml;
...
async function verifyDashboard(resourceName) {
  try {
    const isCost = [].includes(resourceName);
    const isQual = ['LangSmith', 'Ragas', 'AgentOps'].includes(resourceName);
    const isProd = ['OpenTelemetry', 'Apache SkyWalking'].includes(resourceName);
...
async function runEvaluation() {
  ...
  try {
    ...
    const costResources = ["Langfuse", "OpenLIT", "OpenCost"];
    ...
    const prodResources = ["OpenTelemetry", "Apache SkyWalking", "Langfuse", "Workflow Layer"];
    const execResources = ["Langfuse", "Phoenix", "Traceloop", "OpenTelemetry", "Jaeger"];
    const riskResources = ["Falco", "Sentry", "Prometheus"];
    const govResources = ["Detect-Secrets", "Microsoft Presidio", "Open Policy Agent", "Keycloak", "OpenMetadata"];
    ...
    allData = [
      ...
      ...langfuseCostRows,
      ...langfuseExecRows,
      ...execData.filter(r => ['Phoenix', 'Traceloop', 'OpenTelemetry', 'Jaeger'].includes(r.resource_name)),
...
async function loadData() {
  try {
    ...
    const [costData, valData, qualData, prodData, execData, riskData, govData] = await Promise.all([r1.json(), r2.json(), r3.json(), r4.json(), r5.json(), r6.json(), r7.json()]);
    const costResources = ["Langfuse", "OpenLIT", "OpenCost"];
    const valResources = ["DeepEval", "Guardrails AI", "Pydantic AI", "Instructor"];
    const qualResources = ["Ragas", "DeepEval", "AgentOps"];
    const prodResources = ["OpenTelemetry", "Apache SkyWalking", "Langfuse"];
    const riskResources = ["Falco", "Sentry", "Prometheus"];
    const govResources = ["Open Policy Agent", "Keycloak", "OpenMetadata"];
    // Tag Langfuse cost rows so they can be distinguished from execution rows
    const langfuseCostRows = costData
      .filter(r => r.resource_name === 'Langfuse')
      .map(r => ({ ...r, _dimension: 'cost' }));
    ...
    allData = [
      ...
      ...langfuseCostRows,
      ...langfuseExecRows,
      ...execData.filter(r => ['OpenTelemetry', 'Jaeger'].includes(r.resource_name)),
      ...riskData.filter(r => riskResources.includes(r.resource_name)),
      ...govData.filter(r => govResources.includes(r.resource_name))
    ];
    renderGrid();
...
loadUrls().then(() => {
  ...
  setInterval(loadData, 15000);
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (loginTime) {
        const el = document.getElementById('login-time-display');
        if (el) { el.style.display = 'inline-block'; el.innerHTML = '&#128340; <strong style="color:var(--text);">Session Started:</strong> ' + loginTime; }
      }
    });
            ...
            if (e.key === 'Enter') {
                ...
                try {
                    ...
                    const matchedAgent = agents.find(a => 
                        (a.agent_id && a.agent_id.toLowerCase().includes(query)) || 
                        (a.agent_name && a.agent_name.toLowerCase().includes(query))
                    );

                    if (matchedAgent) {
                        window.location.href = '/widget/agent-profile.html?id=' + matchedAgent.agent_id;
                    } else if (['risk', 'productivity', 'quality', 'efficiency', 'governance', 'cost', 'validation'].some(k => query.includes(k))) {
                        window.location.href = '/widget/demo.html?filter=' + encodeURIComponent(query);
                    } else {
                        alert('No direct match found for "' + query + '". Try searching an Agent ID or a Metric.');
                    }
                } catch (err) {
                    console.error(err);
                }
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll('.search-bar').forEach(bar => {
        ...
        // Ensure parent is relative so dropdown positions correctly
        ...
        dropdown.style.background = '#1a1b26'; // dark theme
        ...
        // Show dropdown when clicking back into input if there's text
