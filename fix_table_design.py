...
content = content.replace('<button onclick="addAgent()">Add Agent</button>', '')

# 2. Add Wrapper and Style Table Header
old_table_html = """<table id="agent-table">
      <thead>
        <tr>
          <th>Agent</th>
          <th>Score</th>
          <th>P</th><th>Q</th><th>E</th><th>G</th><th>R</th><th>V</th><th>C</th>
          <th>SME Rating</th>
        </tr>
      </thead>
...
content = content.replace(old_table_html, new_table_html)

# 3. Style JS Rows
old_js_row = """const row = `<tr>
            <td>${a.agent_name}</td>
            <td>${a.score.toFixed(2)}</td>
            <td>${a.metrics.P?.toFixed(2) || '0'}</td>
            <td>${a.metrics.Q?.toFixed(2) || '0'}</td>
            <td>${a.metrics.E?.toFixed(2) || '0'}</td>
            <td>${a.metrics.G?.toFixed(2) || '0'}</td>
            <td>${a.metrics.R?.toFixed(2) || '0'}</td>
            <td>${a.metrics.V?.toFixed(2) || '0'}</td>
            <td>${a.metrics.C?.toFixed(2) || '0'}</td>
            <td style="min-width:120px;" onmouseout="resetStars('${a.agent_id}')">"""
...
print("Table styled matching dashboard!")
