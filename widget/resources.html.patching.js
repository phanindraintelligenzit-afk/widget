...
html = html.replace(
  'const [cUrls, vUrls, qUrls, pUrls, eUrls, rUrls] = await Promise.all([u1.json(), u2.json(), u3.json(), u4.json(), u5.json(), u6.json()]);',
  'const [u1Res, u2Res, u3Res, u4Res, u5Res, u6Res, u7Res] = await Promise.all([u1, u2, u3, u4, u5, u6, arguments[0][6]]);\n      const [cUrls, vUrls, qUrls, pUrls, eUrls, rUrls, gUrls] = await Promise.all([u1Res.json(), u2Res.json(), u3Res.json(), u4Res.json(), u5Res.json(), u6Res.json(), u7Res ? u7Res.json() : Promise.resolve({})]);'
);
// Above hack won't work perfectly. Let's do a precise string replacement on loadUrls()
