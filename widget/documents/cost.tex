...
\begin{titlepage}
    ...
    \begin{center}
        ...
        \begin{tabular}{rl}
            ...
            \textbf{Dimension:} & Cost (C) --- Weight: 5\% \\
...
The Cost dimension provides a normalized score indicating how financially efficient an agent operates relative to a human baseline. Cost carries a weighting of \textbf{5\% (0.05)} on the final composite DPI-LS score.
...
\begin{itemize}
    \item \textbf{\color{dpiblue}\faUserTie\ Human Cost Per Output:} The baseline unit cost to produce one successful output (derived from human salary, benefits, and tools).
    \item \textbf{\color{dpiteal}\faRobot\ AI Cost Per Output:} Calculated as $\frac{\text{Total Model \& Compute Cost}}{\text{Completed Outputs}}$.
    \item \textbf{\color{dpired}\faChartPie\ Utilization:} The percentage $[0.0 - 1.0]$ of provisioned infrastructure that the agent actually utilized, penalizing over-provisioned idle instances.
\end{itemize}
...
\begin{tcolorbox}[enhanced, colback=dpicloud!10!white, colframe=dpicloud, title={\textbf{\faLightbulb\ The Lemonade Stand Analogy}}, fonttitle=\bfseries\large, boxrule=1pt, arc=2mm, drop shadow]
To make this math so simple a kid could understand it, let's pretend you own a lemonade stand:

\vspace{0.2cm}
\begin{itemize}
    \item \textbf{\color{dpiblue}\faUserTie\ Human Cost:} It usually costs you \$10 a day to hire your little brother to squeeze lemons. He makes 10 cups. So, your human baseline is \textbf{\$1.00 per cup}.
    \item \textbf{\color{dpigreen}\faRobot\ AI Cost:} You buy a giant robot to squeeze lemons. The robot costs \$20 a day, but it only makes 5 cups because it's slow. Your robot cost is \textbf{\$4.00 per cup}.
    \item \textbf{\color{dpired}\faBalanceScale\ The Ratio ($\frac{\text{Human}}{\text{Robot}}$):} You divide the human cost (\$1) by the robot cost (\$4). This equals \textbf{0.25 (or 25\%)}. This means your robot is only 25\% as cost-efficient as your brother! If the robot was cheaper than your brother, the math limits the score to exactly \textbf{1.0 (100\%)} so you don't get extra credit just for being cheap.
\end{itemize}
\end{tcolorbox}
...
By querying the Cost Explorer API directly with the corresponding \texttt{agent\_id} tags, the adapter pulls the exact \texttt{UnblendedCost} metric for the given time period, eliminating the need for manual spreadsheet calculations.
...
% -- SCENARIO 1 --
\begin{tcolorbox}[enhanced, colback=dpired!5!white, colframe=dpired, title={\textbf{\faExclamationCircle\ Scenario 1: The Infinite Hallucination Loop (Runaway Spend)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm, drop shadow]
\textbf{\color{dpired}\faFire\ 1. The Event (Financial Black Hole):} 
...
\vspace{0.2cm}
\textbf{\color{dpiblue}\faMoneyBillWave\ 2. FinOps Impact:} 
...
\vspace{0.2cm}
\textbf{\color{dpigreen}\faClock\ 3. Incident Timeline:}
\begin{itemize}
...
\vspace{0.2cm}
\textbf{\color{dpiteal}\faCalculator\ 4. Mathematical Resolution:}
\begin{itemize}
...
% -- SCENARIO 2 --
\vspace{0.5cm}
\begin{tcolorbox}[enhanced, colback=dpicloud!5!white, colframe=dpicloud, title={\textbf{\faServer\ Scenario 2: The Over-Provisioned Idle GPU (Poor Utilization)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm, drop shadow]
\textbf{\color{dpicloud}\faHourglassHalf\ 1. The Event (Wasted Capacity):} 
...
\vspace{0.2cm}
\textbf{\color{dpired}\faMoneyBillWave\ 2. FinOps Impact:} 
...
% ---------------------------------------------------------
% 6. MULTI-CLOUD FINOPS AGGREGATION
% ---------------------------------------------------------
\section{Multi-Cloud FinOps Aggregation}

To accurately calculate the Cost dimension in a hybrid-cloud enterprise, telemetry must be aggregated from multiple billing APIs simultaneously.
...
\begin{tikzpicture}[
    node distance=2.5cm,
    cloud/.style={circle, draw=dpicloud, fill=dpicloud!10, thick, text width=2.5cm, align=center, minimum height=2.5cm, drop shadow},
    agg/.style={diamond, aspect=2, draw=dpiblue, fill=dpiblue!10, thick, text width=3cm, align=center, drop shadow},
...
\node[cloud] (aws) {\textbf{AWS}\\Cost Explorer};
\node[cloud, below=0.5cm of aws] (gcp) {\textbf{GCP}\\Billing};
\node[cloud, below=0.5cm of gcp] (azure) {\textbf{Azure}\\Cost Mgmt};

\node[agg, right=3cm of gcp] (adapter) {DPI-LS\\FinOps Adapter};
\node[engine, right=2cm of adapter] (dpi) {\textbf{DPI-LS Engine}\\Cost (C) Calculation};

\draw[arrow] (aws.east) -- (adapter.north west);
\draw[arrow] (gcp.east) -- (adapter.west);
\draw[arrow] (azure.east) -- (adapter.south west);
\draw[arrow] (adapter.east) -- node[above] {Normalized \$} (dpi.west);
...
% -- SCENARIO 3 --
\begin{tcolorbox}[enhanced, colback=dpigreen!5!white, colframe=dpigreen, title={\textbf{\faChartLine\ Scenario 3: Prompt Caching (Cost Avoidance)}}, fonttitle=\bfseries, boxrule=1pt, arc=2mm, drop shadow]
\textbf{\color{dpigreen}\faChartLine\ 1. The Event (Smart Savings):} 
An agent is deployed behind an LLM Gateway with semantic prompt caching enabled. The agent receives 1,000 requests for the same exact analysis report.

\vspace{0.2cm}
\textbf{\color{dpiblue}\faServer\ 2. FinOps Impact:} 
The gateway serves 950 of the requests directly from the Redis cache. The \texttt{model\_cost} for the 950 cached hits is strictly \$0.00, drastically lowering the average Cost Per Output.

\vspace{0.2cm}
\textbf{\color{dpiteal}\faTrophy\ 3. Mathematical Resolution:}
The AI Unit Cost drops from \$0.50 to \$0.02, maximizing the Cost Ratio and earning a perfect $1.0$ (100\%) score in the DPI-LS Engine. This rewards infrastructure engineering for intelligent cost avoidance.
\end{tcolorbox}
...
\begin{tikzpicture}[
    node distance=3cm,
    phase/.style={circle, draw=white, ultra thick, fill=#1, text=white, minimum size=1.5cm, font=\bfseries\Large, drop shadow},
    box/.style={rectangle, draw=#1, fill=#1!5, thick, rounded corners, text width=4.5cm, align=center, minimum height=2cm, drop shadow},
    line/.style={draw, ultra thick, dpiblue}
]
...
% Phase 1
\node[phase=dpiblue] (p1) at (0,0) {1};
\node[box=dpiblue, above=0.5cm of p1] { \textbf{Weeks 1-2} \\ Strict Tagging Policy (AWS, GCP, Azure) };
\node[box=dpiblue, below=0.5cm of p1] { Enforce \texttt{agent\_id} on resources \& LLM Gateway requests. };
\draw[thick, dpiblue, dashed] (p1) -- ++(0,0.5);
\draw[thick, dpiblue, dashed] (p1) -- ++(0,-0.5);

% Phase 2
\node[phase=dpicloud] (p2) at (6,0) {2};
\node[box=dpicloud, above=0.5cm of p2] { \textbf{Weeks 3-4} \\ Baseline Calibration };
\node[box=dpicloud, below=0.5cm of p2] { Audit human costs and inject into \texttt{human\_cost\_per\_output}. };
\draw[thick, dpicloud, dashed] (p2) -- ++(0,0.5);
\draw[thick, dpicloud, dashed] (p2) -- ++(0,-0.5);

% Phase 3
\node[phase=dpigreen] (p3) at (12,0) {3};
\node[box=dpigreen, above=0.5cm of p3] { \textbf{Weeks 5-6} \\ FinOps Circuit Breaker };
\node[box=dpigreen, below=0.5cm of p3] { Automate agent shutdown \& PagerDuty if $C < 0.40$. };
\draw[thick, dpigreen, dashed] (p3) -- ++(0,0.5);
\draw[thick, dpigreen, dashed] (p3) -- ++(0,-0.5);
...
\textcolor{dpicloud}{\small Digital Performance Index --- Cost Dimension (C) --- Version 2.5}\\[0.1cm]
