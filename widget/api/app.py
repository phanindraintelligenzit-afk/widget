...
def get_cost_evaluation_urls() -> dict[str, dict]:
    ...
    if not prometheus_online:
        # External Prometheus unavailable — use our own /metrics/ endpoint
...
def get_quality_evaluation_urls() -> dict[str, dict]:
    ...
    # Cloud SaaS services — treat as online when API keys are configured
    langsmith_online = bool(os.environ.get("LANGCHAIN_API_KEY"))
    ragas_online = True  # Ragas is a library, always available in-process
    agentops_online = bool(os.environ.get("AGENTOPS_API_KEY"))

    # Fallback: even without keys, allow the button to open the URL
    if not langsmith_online:
        langsmith_online = True
    if not agentops_online:
