...
def test_multi_source_story_one_agent_assembled_from_five_sources(client):
    ...
    assert r_after_puvi["metrics"]["G"] == pytest.approx(0.985, abs=1e-3)
    ...
    assert "G" not in r_final["gate_failures"]
    assert r_final["band"] in ["Needs Optimization", "Underperforming"]
