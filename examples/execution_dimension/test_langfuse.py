...
if __name__ == "__main__":
    ...
    # Create the callback handler for LangGraph
    ...
    # Run the agent natively, injecting the langfuse handler
    ...
    # --- Explicit Execution Dimension Tracking ---
    # We grab the root trace ID generated by the handler
    trace_id = langfuse_handler.last_trace_id
    
    # Evaluate whether the agent was successful
    success_score = 1.0 if response.status == "success" else 0.0
    
    # Post the exact execution status to Langfuse as a 'Score' metric
    if trace_id:
        langfuse_client.create_score(
            trace_id=trace_id,
            name="execution_success",
            value=success_score,
            comment=f"Finished in {response.iterations_used} iterations."
        )
    ...
    if success_score == 1.0:
        print("=> Langfuse Trace Ended: SUCCESS")
    else:
