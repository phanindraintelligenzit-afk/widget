...
if __name__ == "__main__":
    # --- Traceloop / OpenLLMetry Setup ---
    from traceloop.sdk import Traceloop
    from traceloop.sdk.decorators import workflow, task
    Traceloop.init(app_name="ExecutionAgents")
    ...
    @workflow(name="deploy_rds_workflow")
    def run_agent():
        # Run the agent
        ...
        # In Traceloop, if a workflow fails, we explicitly raise an exception so the 
        # OpenTelemetry span automatically registers as an ERROR in the dashboard.
        if response.status != "success":
            raise RuntimeError(f"Agent failed after {response.iterations_used} iterations")
    ...
    try:
        run_agent()
        print("\n" + "="*80)
        print("=> Traceloop Workflow Ended: SUCCESS")
        print("="*80)
    except RuntimeError as e:
        print("\n" + "="*80)
        print(f"=> Traceloop Workflow Ended: FAILED ({e})")
        print("="*80)
