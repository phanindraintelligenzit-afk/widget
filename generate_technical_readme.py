...
* **Deep Linking**: Clickable autocomplete dropdown results route the user instantly to heavily parameterized URLs (e.g. gent-profile.html?id=chandra-finops or demo.html?filter=risk).
...
* **Conditional Score Rendering**: Re-wrote the inner polling loop in dpi-ls.js to dynamically inject inline CSS colors to the final DPI-LS score cells.
  ...
  * Score 51 - 79: **Yellow**
  * Score <= 50: **Red**
* **Strict Configuration Constraints**: Overhauled the gent-config.html UI to accept all 7 specific dimensions (P, Q, E, G, R, C, V). Added hard validation logic ensuring weight distribution equals exactly 100% prior to dispatching Promise.all fetch payloads to the backend.

### 5. Backend Alignment
* **Test Suite Modernization**: Re-wired 	ests/test_widget_serving.py to correctly test the new requirement that unauthenticated root (/) visitors are routed strictly to dmin-login.html instead of the Dashboard.
...
print("README rewritten technically!")
