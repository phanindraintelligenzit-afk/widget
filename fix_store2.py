...
# Replace the broken upsert_agent
broken_start = content.find('def upsert_agent(')
broken_end = content.find('# ---- Task 1')
...
content = content[:broken_start] + new_func + content[broken_end:]

with open('store/repo.py', 'w', encoding='utf-8') as f:
    f.write(content)
print("Fixed store/repo.py completely")
